...
if (isset($_POST['action'])) {
    ...
    if ($action == "getEmployeeData") {
    ...
    } elseif ($action == "getEmployeeAllData") {
    ...
    } elseif ($action == "deletePmeRmeFile") {
    ...
    } elseif ($action == "componentModalForm") {
    ...
    } elseif ($action == "getSectionStationById") {
    ...
    } elseif ($action == "updateFormDuration") {
        if (!isset($_POST['durationValue']) || !isset($_POST['durationId'])) {

            $respo['status'] = false;
            $respo['msg'] = "Duration is not set.";
            echo json_encode($respo);
            die();
        }

        $durationValue = trim($_POST['durationValue']);
        $durationId = trim($_POST['durationId']);

        if (empty($durationValue)) {
            $respo['status'] = false;
            $respo['msg'] = "Duration value required.";
            echo json_encode($respo);
            die();
        }
        ...
        $respo['status'] = false;
        $respo['msg'] = "Something went wrong please try again";
        echo json_encode($respo);
        die();
        ...
        $data=[];
        while($run = mysqli_fetch_object($checkData)){
            $data[] = $run;
