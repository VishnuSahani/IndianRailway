...
$createdDateTime = date("Y-m-d h:i:sa");
    ...
    $check=mysqli_query($con,"select * from assign_incharge_sse_rail where station_id='$stationId'");
    if(mysqli_num_rows($check))
    {
        echo "<span class='text-danger'>Sorry ! This Station Already Assigned to SSE-Incharge</span>";
    }
    else{

        if(mysqli_query($con,"insert into assign_incharge_sse_rail(section_id,station_id,station_name,section_name,empid,empname,empdesg,phone,date_of_app,date_of_ret,created_date)values('$sectionId','$stationId','$stationName','$sectionName','$c','$d','$e','$phone','$appdate','$retdate','$createdDateTime')"))
           {
              echo"success";
