...
<script>
    ...
    if (updateCheck.includes(value)) {
        ...
        displayHtml +=
            `<div id='${element['dac_id']}'><select class="form-control">
        <option>${value}</option>
        <option value='${updateValue[value]}'>${updateValue[value]}</option>
        </select> <button type="button" onclick="updateSingleValue('${updateValue[value]}','DAC3','${element['dac_id']}','${tableDataForm['id']}')" class="btn btn-sm btn-success my-1">Update</button></div>`;
